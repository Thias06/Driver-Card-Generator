...
async function sbGetDriversForDuplicateCheck() {
  const r = await fetch(`${SB}/rest/v1/drivers?select=email,alias`, { headers: { apikey: KEY, Authorization: `Bearer ${KEY}` } });
  const text = await r.text();
  let json = []; try { json = JSON.parse(text || '[]'); } catch { json = []; }
  if (!r.ok) throw new Error((json && (json.message || json.error)) || text || 'Erreur lecture Supabase');
...
async function sbUpload(path, mime, buf) {
  ...
  if (!r.ok) { const t = await r.text(); throw new Error(t || 'Erreur upload Supabase'); }
...
function userEmailHtml(lang, base, logo, link, first) {
  const en = lang === 'en';
  const t = en ? {
...
<table width="600" cellpadding="0" cellspacing="0" role="presentation" style="width:100%;max-width:600px;background:#0b0c12;border-radius:16px;overflow:hidden">
  <tr><td align="center" style="background:#050507;padding:26px 0 18px"><img src="${logo}" width="160" alt="The Ring" style="display:block;border:0"></td></tr>
  <tr><td style="height:5px;background:linear-gradient(90deg,#34b8ff,#2e54ff,#7a33f0,#e22ed0);font-size:0;line-height:0">&nbsp;</td></tr>
  <tr><td style="padding:30px 30px 8px"><div style="font-family:Arial,sans-serif;font-weight:800;font-size:30px;color:#ffffff;text-transform:uppercase">${t.hi}</div></td></tr>
  <tr><td style="padding:6px 30px 0">
    <p style="margin:0 0 16px;color:#c9cbd8;font-size:16px;line-height:1.55;font-family:Arial,Helvetica,sans-serif">${t.welcome}</p>
    <table cellpadding="0" cellspacing="0" role="presentation" align="center" style="margin:6px auto 22px"><tr><td style="border-radius:10px;background-image:linear-gradient(90deg,#2e54ff,#7a33f0,#e22ed0)">
      <a href="${link}" style="display:inline-block;padding:15px 30px;font-family:Arial,Helvetica,sans-serif;font-weight:bold;font-size:14px;letter-spacing:1px;color:#ffffff;text-decoration:none">${t.btn}</a>
    </td></tr></table>
    <p style="margin:0 0 16px;color:#c9cbd8;font-size:15px;line-height:1.55;font-family:Arial,Helvetica,sans-serif">${t.back}</p>
    <p style="margin:0 0 6px;color:#c9cbd8;font-size:15px;line-height:1.55;font-family:Arial,Helvetica,sans-serif">${t.invite}</p>
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (!p.first || !p.last || !cleanAlias || !cleanEmail) {
      return res.status(400).json({ error: en ? 'Missing required fields.' : 'Champs obligatoires manquants.' });
    }
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(cleanEmail)) {
      return res.status(400).json({ error: en ? 'Invalid email address.' : 'Adresse email invalide.' });
    }
    const ageNum = p.age ? parseInt(p.age, 10) : null;
    if (ageNum !== null && (isNaN(ageNum) || ageNum < 16 || ageNum > 99)) {
      return res.status(400).json({ error: en ? 'Age must be a number between 16 and 99.' : "L'âge doit être un nombre entre 16 et 99." });
    }

    const existing = await sbGetDriversForDuplicateCheck();
    const sameEmail = existing.some(x => String(x.email || '').trim().toLowerCase() === cleanEmail);
    const sameAlias = existing.some(x => String(x.alias || '').trim().toLowerCase() === cleanAliasLower);

    if (sameEmail && sameAlias) return res.status(409).json({ error: en ? 'This nickname and email are already used.' : 'Ce pseudo et cette adresse email sont déjà utilisés.' });
    if (sameEmail) return res.status(409).json({ error: en ? 'This email address is already used.' : 'Cette adresse email est déjà utilisée.' });
    if (sameAlias) return res.status(409).json({ error: en ? 'This nickname is already used.' : 'Ce pseudo est déjà utilisé.' });

    const slug = slugify(cleanAlias) || slugify(p.last) || 'pilote';

    let photo_url = '', card_url = '';
    const photo = dataUrlToBuffer(body.photo);
    if (photo) { const ext = photo.mime.includes('png') ? 'png' : 'jpg'; photo_url = await sbUpload(`photo/${slug}.${ext}`, photo.mime, photo.buf); }
    const card = dataUrlToBuffer(body.card);
    if (card) card_url = await sbUpload(`card/${slug}.png`, 'image/png', card.buf);

    const row = {
      status: 'published', hidden: false,
      first_name: p.first, last_name: p.last, alias: cleanAlias,
      age: ageNum, nationality: p.nationality || '', city: p.city || '',
      driving_style: p.style || '', email: cleanEmail,
      photo_url, card_url, slug, equipment: p.equipment || '',
      language: lang, season: 'S0', overall: 50, level: 'ROOKIE'
    };
    ...
    const teamHtml = `<h2>Nouvelle inscription — Season 0</h2>
      ...
      <b>Âge :</b> ${ageNum || '-'}<br><b>Ville :</b> ${p.city || '-'} · ${p.nationality || '-'}<br>
      <b>Langue :</b> ${en ? 'EN' : 'FR'}<br><b>Style :</b> ${p.style || '-'}<br><b>Email :</b> ${cleanEmail}<br>
    ...
    await sendEmail(TEAM, `🏁 Inscription Season 0 — ${cleanAlias}`, teamHtml,
      cardB64 ? [{ filename: `TTR_S0_${slug}.png`, content: cardB64 }] : undefined).catch(() => {});

    const subject = en ? 'Welcome to The Ring League 🏁' : 'Bienvenue dans The Ring League 🏁';
    await sendEmail(cleanEmail, subject, userEmailHtml(lang, base, logo, link, p.first)).catch(() => {});
