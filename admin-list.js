...
// Page publique de la carte, rendue depuis les données (modifiable via l'admin).
...
module.exports = async (req, res) => {
  ...
  if (!row) {
    return res.status(404).send(`<!doctype html><meta charset="utf-8"><body style="background:#050507;color:#9092a4;font-family:sans-serif;text-align:center;padding:80px">Carte introuvable / Card not found.</body>`);
  }
  ...
  const title = esc(`${row.alias} — The Ring League · Season 0`);
...
${og ? `<meta property="og:image" content="${esc(og)}"><meta name="twitter:card" content="summary_large_image"><meta name="twitter:image" content="${esc(og)}">` : ''}
...
<script>
var D=${JSON.stringify(D).replace(/</g,'\\u003c')}; var P=${JSON.stringify(row.photo_url || '').replace(/</g,'\\u003c')};
